...
class MockSession:
    ...
    @staticmethod
    def list_channels(test_list_channels_data):
        channel_list = test_list_channel_data()
        return channel_list

    # mock get_build returns test data to mock session.getBuild()
    @staticmethod
    def get_build(build_id, mock_get_build_dict):
        """
        returns the test build info for a given build id
        """
        build_dict = mock_get_build_dict
        return build_dict[build_id]

    # returns test data to mock session.getBuildLogs()
    @staticmethod
    def get_build_logs(build_id, mock_get_build_logs_dict):
        """
        returns the test build log info for a given build
        """
        log_dict = mock_get_build_logs_dict
        return log_dict[build_id]

    @staticmethod
    def requests_get(url, mock_requests_get_log_dict):
        """
        returns test log string for requests.get(url) for log collection
        """
        response_dict = mock_requests_get_log_dict
...
def test_collect_hw_info(mock_session_response, test_host_with_build):
    """
    ...
    my_host.get_logs_for_host(session)

    assert my_host.hw_dict == test_94_hw_dict


@pytest.fixture
def test_host_with_build():
    """
    ...
    test_host = cv.host("rhel8", 94, True)
